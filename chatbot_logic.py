...
def get_emotion_message(intensity):
    """감정 강도에 따른 메시지 반환"""
    if intensity <= 3:
        return "지금 느끼는 감정이 약한 편이네요. 함께 편안하게 이야기 나눠봐요. 😊"
    elif intensity <= 6:
        return "지금 중간 정도의 감정을 느끼고 계시는군요. 천천히 함께 살펴볼게요. 🤗"
    else:
        return "지금 강한 감정을 느끼고 계시는군요. 충분히 이해합니다. 함께 차근차근 이야기 나눠봐요. 💪"
...
- 성별: {user_info['gender']}
- 나이: {user_info['age']}세
- 학교급: {user_info['school_level']}
- 현재 감정 강도: {user_info['emotion_intensity']}/10
